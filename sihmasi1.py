...
def extract_metadata(file_path):
    ...
    try:
        # Get absolute path to ensure we're reading the correct file
        abs_path = os.path.abspath(file_path)
        
        # Run exiftool with explicit file path
        result = subprocess.run(
            ['exiftool', '-a', '-G1', '-s', abs_path],
            capture_output=True,
            text=True,
            check=True,
...
def extract_gps_data(metadata):
    ...
    for line in metadata.split('\n'):
        ...
        if 'gpslatitude' in line_lower and 'ref' not in line_lower:
            if ':' in line:
                gps_data['latitude'] = line.split(':', 1)[1].strip()
        elif 'gpslongitude' in line_lower and 'ref' not in line_lower:
            if ':' in line:
                gps_data['longitude'] = line.split(':', 1)[1].strip()
        elif 'gpsaltitude' in line_lower and 'ref' not in line_lower:
            if ':' in line:
                gps_data['altitude'] = line.split(':', 1)[1].strip()
        elif 'gpsposition' in line_lower:
            if ':' in line:
                gps_data['position'] = line.split(':', 1)[1].strip()
...
def display_metadata(file_path, metadata, save_output=False):
    ...
    if gps_data:
        ...
        print(f"\n{YELLOW}[*] No GPS data found in this file{RESET}")
